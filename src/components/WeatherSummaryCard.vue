...
<template>
  <div class="relative flex flex-col flex-wrap gap-2 w-4/5 overflow-hidden">
    <h1 class="font-semibold transition-colors dark:text-slate-200">Searching Summary</h1>
    <TransitionGroup name="list" tag="ul" class="flex gap-6 relative">
      <div v-if="!summarySearch.length" class="bg-white h-80 w-full flex justify-center items-center card-temporary rounded-3xl transition-colors dark:bg-[#292727]">
        <h1 class="text-slate-600 transition-colors dark:text-slate-200">Your Latest Searching will appear...</h1>
      </div>
      <li
      ...
      >
        <div class="absolute opacity-0 w-full h-full transition-all group-hover/item:opacity-100 bg-white/[.85] dark:bg-[#292727]/[.85]">
